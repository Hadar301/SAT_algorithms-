...
def RandomPlantedSAT_test(n,c, test_it, algorithm):
    ...
    m = np.ceil(n*c)
    #res_arr_wp = np.zeros(10)
    #res_arr_bp = np.zeros(10)
    #res_arr_sp = np.zeros(10)
    res_arr = np.zeros(10)
    for i in range(T//10):
        print("------n={}--c={}---test segment# {}---itertation# {}---\n".format(n,c,test_it,i))
        prop = RandomPlantedSAT(n,m,k,max_iters) #prop_wp = RandomPlantedSAT(n,m,k,max_iters)
        #prop_bp = copy.deepcopy(prop_wp)
        #prop_sp = copy.deepcopy(prop_wp) 
...
def test_results(prop, start):
    ...
    return np.array([SAT_counter, num_of_SAT_clauses, hamming_distnace, runtime, dont_care_conter, iteration_counter, absent_literals, 
                     majorityVote_sat_counter, majorityVote_num_of_SAT_clauses, majorityVote_hamming_distnace])
...
def parse_results():
    ...
    for key in res_dict:
        ...
        df = df.append({'N and C' : (n,c), 'SAT Percentage': res_arr[0]*100, 'Average Percentage of SAT Clauses':100*res_arr[1]/m, 
                                  'Average Percent Hamming distance': 100*res_arr[2]/n, 'Average Runtime': res_arr[3]/3600, 'Average Percent Dont Care': res_arr[4]/n, 
                                  'Average Number of Iterations': res_arr[5], 'Average number of absent literals': res_arr[6]/n,
                                  'SAT Percentage (MV)': res_arr[7]*100, 'Average Percentage of SAT Clauses(MV)': 100*res_arr[8]/m,
                                  'Average Percent Hamming distance (MV)': 100*res_arr[9]/n}, ignore_index=True)
