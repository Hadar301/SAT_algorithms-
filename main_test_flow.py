# -*- coding: utf-8 -*-
"""
Created on Mon Jan  3 10:39:21 2022
...
def SAT_test(n,c, test_it, algorithm, class_type):
    ...
    for i in range(T//10):
        ...
        prop = class_type(n,m,k,max_iters) #RandomPlantedSAT(n,m,k,max_iters)
...
def test_flow(n,c, test_it, algorithm, class_type):
    #res_arr = []
    res_arr = SAT_test(n,c, test_it, algorithm, class_type)
...
def accumulate_results_per_iteration(n, c, res_arr, algorithm):
    ...
    while(global_lock_f2.locked()):
        continue
    global_lock_f2.acquire()
...
def parse_results(class_type):
    ...
    df_per_iteration = pd.DataFrame(columns=['N and C', 'SAT Bit', 'Number of SAT Clauses', 'Hamming distance',
                                  'Runtime', 'Number of Dont Care', 'Number of Iterations', 'number of absent literals',
                                  'SAT Bit (MV)', 'Number of SAT Clauses(MV)', 'Hamming distance (MV)', 
    ...
    for key in res_dict:
        ...
        file_name = str(class_type.__name__)+'_'+ str(algorithm)+'_n_'+str(n)+'.csv'  #'randomPlanted_SAT_'+ str(algorithm)+'_n_'+str(n)+'.csv'       
    ...
    for key in res_per_itration_dict:
        ...
        file_name_per_res = str(class_type.__name__)+'perIter'+'_'+ str(algorithm)+'_n_'+str(n)+'.csv' #'randomPlanted_SAT_perIter'+ str(algorithm)+'_n_'+str(n)+'.csv'
        print(file_name_per_res)
        for res_arr in res_per_itration_dict[key]:
            df_per_iteration = df_per_iteration.append({'N and C' : (n,c), 'SAT Bit': res_arr[0], 'Number of SAT Clauses':res_arr[1], 
                                  'Hamming distance': res_arr[2], 'Runtime': res_arr[3]/3600, 
                                  'Number of Dont Care': res_arr[4], 'Number of Iterations': res_arr[5], 
                                  'number of absent literals': res_arr[6], 'SAT Bit (MV)': res_arr[7], 
                                  'Number of SAT Clauses(MV)': res_arr[8], 'Hamming distance (MV)': res_arr[9], 
                                  'Number of Contradictions (WP)': res_arr[10]}, ignore_index=True)
